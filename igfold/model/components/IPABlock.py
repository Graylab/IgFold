...
def full_precision(device):
    """Context that disables TF32 and autocast on CUDA; a no-op on other devices."""
    if device.type == "cuda":
        return _cuda_full_precision()
    return nullcontext()


@contextmanager
def _cuda_full_precision():
    with disable_tf32(), torch.autocast(device_type="cuda", enabled=False):
        yield
    ...
    def forward(
        ...
        *,
        ...
        if exists(key_padding_mask):
            # padded keys get exactly zero weight, even when `mask` excludes every key
            # (which gives uniform attention over the remaining keys)
            key_padding_mask = repeat(key_padding_mask, "b j -> (b h) () j", h=h)
        ...
        with full_precision(attn.device):
            # disable TF32 / autocast for precision
            ...
            results_points = einsum(
                "b n d c, b n c r -> b n d r", results_points - translations, rotations.transpose(-1, -2)
            )
